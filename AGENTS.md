...
Partition tables are **not** static CSVs anymore. Before every build,
`cmake/generate_partitions.py` runs and produces
`apps/<app>/partitions_<app>.csv` sized so that:

- `factory` (app) is sized to the **actual `.bin` size + 128 KB margin** when a
  previous build produced a `.bin`; otherwise it falls back to 1 MB.
- `storage` (SPIFFS) starts right after `factory` and runs to the end of flash.
- The remaining free flash after the app is fully used for data storage.

Flash size is read from each app's `sdkconfig` (`CONFIG_ESPTOOLPY_FLASHSIZE`):
the client is 4 MB, the server is 16 MB.

### Two-pass build wrapper

Because the first pass has no `.bin` to measure, the project ships a wrapper
that does two `idf.py build` invocations in sequence:

| Action | Command |
|---|---|
| Build (client, two-pass) | `scripts/build.sh client` |
| Build (server, two-pass) | `scripts/build.sh server` |
| Flash (client) | `scripts/flash.sh client` |
| Flash (server) | `scripts/flash.sh server` |

Pass 1 builds with a 1 MB factory fallback. Pass 2 reads
`apps/<app>/build/<app>.bin`, regenerates the partition CSV with the tight
factory size, and rebuilds (incremental).

A plain `idf.py build` also works: the CMake hook in
`apps/<app>/CMakeLists.txt` re-invokes the generator on every configure
using whatever `.bin` is already in `build/`. First-ever build uses 1 MB,
subsequent builds use the actual size.

### Direct idf.py commands

If you bypass the wrapper (not recommended for production), the standard
commands still work but will not produce the tightest partition table until
you run the wrapper or the second `idf.py build`:
...
### Runtime ring buffer capacity

Both apps compute their ring buffer size at boot from the actual SPIFFS
partition size, so the storage fills all the flash that was allocated to it:

- Client: `max_records = (spiffs_total - 32 KB reserve - header) / sizeof(record)`.
  On 4 MB flash with ~3 MB SPIFFS this is ~136,000 records (was 5,000).
- Server: `max_records_per_client = (spiffs_total - 32 KB reserve) /
  DATA_STORE_MAX_CLIENTS / (sizeof(record) + header)`.
  On 16 MB flash with ~14 MB SPIFFS this is ~65,000 records per client (was 10,000).

When flashing a new image that changes the partition layout, run
`erase_flash` first so the SPIFFS on the device is reformatted to the new
size and the per-file ring buffer header is reinitialised.
...
│       └── sdkconfig
│   (partition tables for both apps are auto-generated by
│    cmake/generate_partitions.py at build time)
...
| Flash | **4 MB for client, 16 MB for server** (QIO mode, set via `CONFIG_ESPTOOLPY_FLASHSIZE`) |
...
- Flash size: **4 MB for client, 16 MB for server** (auto-detected by `boot:` from partition table)
